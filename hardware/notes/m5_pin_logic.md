...
| G7 | UART RX | Receives ESP32-CAM status output through the installed resistor. |
| G8 | UART TX | Reserved for the later command path to the ESP32-CAM. |
...
The StickS3 Hat2 header gives us enough exposed GPIO for a UART status link. The current test uses G7 as StickS3 RX.

Use this crossover wiring:

| M5StickS3 | ESP32-CAM | Purpose |
|:---|:---|:---|
| G7 / RX | U0T / GPIO1 | ESP32-CAM sends status/log lines through the installed resistor. |
| G8 / TX | U0R / GPIO3 | Optional future command path from StickS3 to ESP32-CAM. |
...
The current integration test uses `G7` and `GND` for ESP32-CAM-to-StickS3 status. `G8` is reserved for a future command path, and `G5` is reserved for a later version where the StickS3 owns the PIR input.
